import { PLANT_TYPES, STORAGE_TYPES, PROFILE_TYPES, OFFLINE } from './constants'

// Flat two-tone building icons on a shared 48x48 canvas -- one main color per icon (the plant's,
// storage kind's or building profile's own color from constants.js) with black/white overlays for
// shading and windows, so the whole set stays consistent without carrying any image assets.
// Purely presentational: no state, no data, safe to memo around.
...
function SolarIcon({ c, size }) {
  return (
    <Svg size={size}>
      <circle cx="37" cy="11" r="6" fill="#fde047" />
...
/** The blades only spin for a plant that is actually generating -- see WindIcon's `spinning`. */
...
function BatteryIcon({ c, size }) {
  return (
    <Svg size={size}>
      <rect x="19" y="6" width="10" height="5" rx="1.5" fill={c} />
      <rect x="12" y="10" width="24" height="32" rx="4" fill={c} />
      <rect x="12" y="10" width="24" height="32" rx="4" {...SHADE} fillOpacity="0.12" />
      <polygon points="26,15 18,27 23,27 21,37 30,24 25,24" {...LIGHT} />
    </Svg>
  )
}

function HydrogenIcon({ c, size }) {
  return (
    <Svg size={size}>
      <rect x="8" y="14" width="32" height="22" rx="11" fill={c} />
      <rect x="8" y="14" width="32" height="22" rx="11" {...SHADE} fillOpacity="0.12" />
      <rect x="20" y="9" width="8" height="6" rx="1.5" fill={c} />
      <rect x="20" y="9" width="8" height="6" rx="1.5" {...SHADE} />
      <text x="24" y="30.5" textAnchor="middle" fontSize="12" fontWeight="700" fill="#fff" fontFamily="sans-serif">H2</text>
      <rect x="12" y="36" width="5" height="6" rx="1" fill={c} />
      <rect x="31" y="36" width="5" height="6" rx="1" fill={c} />
    </Svg>
  )
}

/** A generation or storage unit. `spinning` only matters for wind (the blades). */
export function SourceIcon({ kind, type, size = 40, spinning = false }) {
  if (kind === 'storage') {
    const c = STORAGE_TYPES[type]?.color ?? OFFLINE
    return type === 'HYDROGEN' ? <HydrogenIcon c={c} size={size} /> : <BatteryIcon c={c} size={size} />
  }
...
export function ZoneIcon({ color = '#059669', size = 20 }) {
  return (
    <Svg size={size}>
