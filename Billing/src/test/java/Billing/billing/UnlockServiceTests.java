...
class UnlockServiceTests {
    ...
    void cumulativeSoldIsRawPlusRolledUp() {
        ...
        var rollups = mock(BillingRollupQuery.class);
        given(records.sumKwhSince(any())).willReturn(12_000.0);
        given(rollups.sumTotalKwhBefore(any())).willReturn(38_000.0);
    ...
    void aTypeIsUnlockedOnceItsThresholdIsReached() {
        var records = mock(BillingRecordRepository.class);
        var rollups = mock(BillingRollupQuery.class);
        given(records.sumKwhSince(any())).willReturn(50_000.0);
        given(rollups.sumTotalKwhBefore(any())).willReturn(0.0);
        var service = new UnlockService(records, rollups);
    ...
    void nothingLockedReportsEmpty() {
        var records = mock(BillingRecordRepository.class);
        var rollups = mock(BillingRollupQuery.class);
        given(records.sumKwhSince(any())).willReturn(1_000_000.0);
        given(rollups.sumTotalKwhBefore(any())).willReturn(0.0);
        var service = new UnlockService(records, rollups);
